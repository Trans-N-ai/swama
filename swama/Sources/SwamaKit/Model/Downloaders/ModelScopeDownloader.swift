...
public class ModelScopeDownloader: IDownloader {
    ...
    public func getWhisperKitFileSize(url: String) async throws -> Int64 {
        ...
        do {
            ...
            // match the file name is [fileRealName], return the file size
            let file = json.first { dict -> Bool in
                guard let path = dict["Path"] as? String else { return false }
                return path == fileName
            }

            return (file?["Size"] as? NSNumber)?.int64Value ?? 0
        }
        catch {
            throw NSError(
                domain: "ModelScopeDownloader",
